...
#             # Fetch existing record IDs in the cloud database for quick lookup
#             cloud_cursor.execute("SELECT id FROM LiquidPlant;")
#             cloud_ids = [r[0] for r in cloud_cursor.fetchall()]
...
#                         INSERT INTO LiquidPlant (id, machines_id, department_id, product_id, defects_id, image, plant_id, recorded_date_time)
...
#         # Initialize dictionaries to store defect counts
#         defect_counts_today = defaultdict(int)
#         defect_counts_other_days = defaultdict(lambda: defaultdict(int))

#         # Aggregate defect counts for today and other days
...
#                     # Here you can add logic if needed for parameters graph
#                     pass
#                 else:
#                     graph_date = graph_datetime.date()
#                     defect_counts_other_days[str(graph_date)]['Parameter Counts'] += 1
#             except ValueError:
#                 pass  # Handle invalid datetime format gracefully

#         # Prepare the final report structure
#         daily_defects_report = {}
#         if defect_counts_today:
#             daily_defects_report[str(today)] = dict(defect_counts_today)
#         daily_defects_report.update(defect_counts_other_days)
...
#             'defect_data': daily_defects_report.get(str(today), {}),
#         })
